...
def calculate_similarity_index():
    ...
    yy = np.arange(1999,2020) #np.arange(1979,2020)
    for year in yy: 
        print(year)
        ...
        for ix, x in enumerate(xx):
            ...
            for iy, y in enumerate(yy): 
                ...
                corv[ix,iy]  = np.corrcoef(x.values,y.values)[1,0]
...
for ik, k in enumerate(season[:]):
    ...
    d1 = xr.open_dataset('output/'+k+'/orgdata.nc')
    ax = plt.axes([.05+ik*.25,.15,.19,.7])   
    label = {'sim':'S-SIM', 'ed':'ED','cor':'COR'}
    color = {'sim':'r', 'ed':'g','cor':'b'}
    for sim in simi[:]:
        x = ds[sim].values #.isel(n1=bmuind[u1],n2=bmuind[u2])
        xx = d1.PMSL.values[:] / 100.
        simv = np.zeros([len(xx), len(xx)])
        for ix, x in enumerate(xx[:]):
            print(ix)
            for iy, y in enumerate(xx):         
                if sim == 'sim': simv[ix,iy] = strsim(x, y)
                if sim == 'ed': simv[ix,iy]  = -np.linalg.norm(x - y)
                if sim == 'cor': simv[ix,iy]  = np.corrcoef(x.flatten(),y.flatten())[1,0] 
        print(simv[0])
        ...
        do = xr.Dataset()
        do['sim'] = (('from', 'to'), simv)
        do.coords['from'] = (('from'),d1.time)
        do.coords['to'] = (('to'),d1.time)       
        odir = 'output/'+k+'/'
        if not os.path.exists(odir): os.makedirs(odir)          
        do.to_netcdf(odir+'orgdata_'+sim+'_ss.nc')
...
plt.savefig('output/fig_hist_rev.png', dpi=150)
