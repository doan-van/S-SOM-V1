...
if __name__ == "__main__":
    # Importing numpy for data generation
    import numpy as np
    import matplotlib.pyplot as plt

    # Initialize the SSOM instance
    som = SSOM(grid_rows=5, grid_cols=5, num_iterations=100, max_learning_rate=0.1, 
               learning_decay=0.5, neighborhood_decay=1.0, metric="euclidean")
    
    # Generate some sample data with 10 samples, each of 3 dimensions
    input_data = np.random.rand(10, 3)

    # Print initial state of the SOM grid (randomly initialized weights)
    print("Initial SOM Grid:")
    print(som.som_grid)

    # Train the SOM on the generated data
    sample_bmu_mapping = som.train(input_data)

    # Print final state of the SOM grid after training
    print("\nTrained SOM Grid:")
    print(som.som_grid)

    # Print the Best Matching Unit (BMU) mappings for each sample
    print("\nSample BMU Mapping:")
    for sample_index, bmu_index in sample_bmu_mapping:
        print(f"Sample {sample_index} mapped to BMU at position {bmu_index}")
